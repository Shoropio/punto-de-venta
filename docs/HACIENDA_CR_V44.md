...
Facturacion electronica completa: generacion XML, firma digital, envio a Hacienda y consulta de estado.
...
### XML y firma
- Generacion de XML v4.4 y almacenamiento en `storage/app/private/hacienda/xml`.
- Firma XMLDSig RSA-SHA256 con certificado `.p12` y almacenamiento en `storage/app/private/hacienda/signed`.

### Envio y consulta
- Autenticacion OAuth2/OIDC contra Hacienda con credenciales ATV.
- Envio del XML firmado a recepcion.
- Consulta de estado por clave y almacenamiento de respuesta XML.
- Normalizacion de estados: `'aceptado'` -> `'accepted'` (via `HaciendaApiClient::normalizeStatus()`).

### PDF
- Generacion de PDF con DomPDF (`barryvdh/laravel-dompdf`) a traves de `PdfService`.

### Endpoints
- `POST /api/invoices/{invoice}/xml` - Regenerar XML
- `POST /api/invoices/{invoice}/sign` - Firmar XML
- `POST /api/invoices/{invoice}/submit` - Enviar a Hacienda
- `POST /api/invoices/{invoice}/status` - Consultar estado

### UI
- Configuracion de Hacienda en SettingsModule con selector de ambiente, datos fiscales y estado de WhatsApp.
